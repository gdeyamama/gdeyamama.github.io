...
const awaitModal = (cb, afterClose) => {
...
const checkUserOrAuth = async (reason = 'Для продолжения операции') => new Promise((res, rej) => {
  if (window.auth?.user) {
    return res(window.auth.user);
  }
  


  awaitModal((handleClose) => {

    const handleAuth = (e) => {
      if (e.detail.user) {
        handleClose(true, e.detail.user);
    ...
    return crEl(
      ...
      crEl('p', {s:{textAlign:'center'}},crEl('small', {s:{ opacity:'0.5' }}, 'Убедитесь что всплывающие окна не блокируются вашим браузером')),
